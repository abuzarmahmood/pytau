...
def plot_changepoint_raster(spike_array, tau, plot_lims=None):
    ...
    fig, ax = plt.subplots(n_trials, 1, figsize=(15, 5 * n_trials), sharex=True, sharey=True)
    for trial in range(n_trials):
        ...
        for change in range(n_states - 1):
            ax[trial].axvline(tau[trial, change], color="r", linestyle="--", linewidth=2)
        for state in range(n_states):
            if state == 0:
                ax[trial].axvspan(0, tau[trial, state], color=cmap(state), alpha=0.2)
            elif state == n_states - 1:
                ax[trial].axvspan(tau[trial, state - 1], n_bins, color=cmap(state), alpha=0.2)
            else:
                ax[trial].axvspan(
                    tau[trial, state - 1],
                    tau[trial, state],
    ...
    for transitions in range(n_transitions):
        ...
        for neuron in range(n_neurons):
            if significant_neurons[neuron, transitions]:
                ax[transitions].axhspan(neuron - 0.5, neuron + 0.5, color="y", alpha=0.5, zorder=10)
    return fig, ax
...
def plot_state_firing_rates(spike_array, tau):
    ...
    fig, ax = plt.subplots(n_states, 1, figsize=(10, 3 * n_states), sharex=True, sharey=True)
    for state in range(n_states):
        ax[state].bar(
            np.arange(n_neurons),
        ...
        for neuron in range(n_neurons):
            if neuron in significant_neurons:
                ...
                ax[state].axvspan(neuron - 0.5, neuron + 0.5, color="y", alpha=0.5, zorder=10)
