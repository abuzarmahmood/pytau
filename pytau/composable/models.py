...
class ComposableModel(ChangepointModel):
    ...
    def generate_model(self):
        ...
        with pm.Model() as model:
            ...
            tau, weight_stack = self.changepoint_component.generate_changepoints(time_length)
    ...
    def test(self):
        ...
        if hasattr(self.changepoint_component, 'n_states'):
            ...
            n_states = min(3, self.changepoint_component.max_states)  # Use 3 for testing
        else:
            n_states = 3
            
        # Generate test data matching the original data shape
        if len(self.data_array.shape) == 3:
            test_data = gen_test_array((5, 10, 100), n_states=n_states, type=data_type)
        elif len(self.data_array.shape) == 2:
            test_data = gen_test_array((10, 100), n_states=n_states, type=data_type)
        else:
