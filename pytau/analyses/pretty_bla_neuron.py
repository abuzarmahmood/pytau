...
nrn_state_firing = []
nrn_scaled_tau = []
for spikes, firing, tau in tqdm(zip(all_raw_spikes, all_state_firing, all_scaled_tau)):
...
for this_ind, this_spikes, this_tau in zip(max_increase_inds, inc_spikes, inc_tau):
    fig, ax = plt.subplots()
    ax = vz.raster(ax, this_spikes, marker="|")
    ax.scatter(this_tau, np.arange(len(this_tau)), color="red", alpha=0.7)
    ax.set_xlim(plot_lims)
