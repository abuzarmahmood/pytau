...
wanted_frame = pd.read_json(
        os.path.join(data_dir, 'bla_population_elbo_repeat.json'))
...
wanted_frame.to_json(os.path.join(data_dir,
            f'{wanted_exp_name}_elbo.json'))
...
# ANOVA
anova_results = pg.rm_anova(data = analysis_frame,
                dv = 'model_elbo', 
                within = ['model.states','preprocess.data_transform'],
                subject = 'data.basename')
anova_results.to_csv(
        os.path.join(data_dir, f'two_way_rm_anova_{wanted_exp_name}.csv'))
...
fig.savefig(os.path.join(data_dir, 
    f'{wanted_exp_name}_elbo_shuffle_comparison.svg'), 
    format = 'svg', dpi = 300)
plt.close(fig)
...
sns.scatterplot(data = median_ranks,
                x = 'plot_states',
                y = 'rank',
                hue = 'transform', ax = ax,
                s = 100,
                ...
                linewidth = 2,
                zorder = 10)
sns.lineplot(data = median_ranks,
                x = 'plot_states',
                y = 'rank',
                hue = 'transform', ax = ax,
                linewidth = 2,
...
#plt.show()
fig.savefig(os.path.join(data_dir, 
    f'{wanted_exp_name}_elbo_ranks.svg'), format = 'svg', dpi = 300)
plt.close(fig)
...
actual_dat = analysis_frame[
        analysis_frame['preprocess.data_transform'] == 'None']
actual_dat['state_ranks'] = \
        actual_dat.groupby('data.basename')['model_elbo'].rank()
# Calculate median rank for each state
median_state_ranks = \
        actual_dat.groupby('model.states')['state_ranks'].median()


fig,ax = plt.subplots(figsize = (3,2))
sns.swarmplot(data = actual_dat,
        x = 'model.states', y = 'state_ranks',
        ax=ax, color = 'grey')
        #jitter = 0.25, ax=ax)
sns.lineplot(data = median_state_ranks,
...
#plt.show()
fig.savefig(os.path.join(data_dir, 
    f'{wanted_exp_name}_state_ranks.svg'), format = 'svg', dpi = 300)
plt.close(fig)
#plt.legend()
