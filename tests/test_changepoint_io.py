...
class TestFitHandler(unittest.TestCase):
    ...
    def test_run_inference(self, mock_compile_wait, mock_ephys_data):
        ...
        mock_model = Mock()
        mock_model.fit.return_value = {'model': 'fitted_model', 'trace': 'trace_data'}
        handler.model = mock_model
        
        # Mock required attributes
        handler.model_params = {'fit': 'advi', 'samples': 1000}
        mock_inference_func = Mock()
        mock_inference_func.return_value = {'model': 'fitted_model', 'trace': 'trace_data'}
    ...
    def test_save_fit_output(self, mock_ephys_data):
        ...
        with tempfile.TemporaryDirectory() as temp_dir:
            ...
            mock_db_handler.model_save_path = os.path.join(temp_dir, 'test_model')
            mock_db_handler.aggregate_metadata.return_value = {'test': 'metadata'}
            handler.database_handler = mock_db_handler
            
            # Mock inference outputs and required attributes
            handler.inference_outs = {'model': 'fitted_model', 'trace': 'trace_data'}
            handler.preprocess_params = {'bin_width': 10}
            handler.model_params = {
            'states': 3,
            'model_kwargs': {'param1': 'value1'}
