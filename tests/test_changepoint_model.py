...
def test_advi_fit():
    ...
    # model, trace, lambda_stack, tau_samples, observations
    assert len(result) == 5
...
def test_mcmc_fit():
    ...
    from pytau.changepoint_model import SingleTastePoisson, mcmc_fit

    # Generate test data
    test_data = gen_test_array(
        (3, 5, 50), n_states=2, type="poisson")  # Smaller for speed
...
def test_dpp_fit():
    ...
    from pytau.changepoint_model import SingleTastePoisson, dpp_fit
...
def test_sampler_with_gaussian_model():
    ...
    # model, trace, mu_stack, sigma_stack, tau_samples, observations
    assert len(result) == 6
...
def test_dpp_fit_with_numpyro():
    ...
    try:
        ...
        trace = dpp_fit(model, n_chains=2, n_cores=1,
                        tune=5, draws=5, use_numpyro=True)
