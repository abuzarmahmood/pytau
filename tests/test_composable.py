...
from pytau.composable import (
    FixedChangepoint, DirichletChangepoint,
    PoissonEmission, GaussianEmission,
    PoissonLikelihood, GaussianLikelihood,
    ComposableModel,
...
class TestEmissionComponents:
    ...
    def test_gaussian_emission_mean_only(self):
        ...
        emission = GaussianEmission(data.shape, n_states, include_variance=False)
    ...
    def test_gaussian_emission_mean_var(self):
        ...
        emission = GaussianEmission(data.shape, n_states, include_variance=True)
...
class TestLikelihoodComponents:
    ...
    def test_poisson_likelihood_2d(self):
        ...
        with pm.Model():
            ...
            lambda_vals = pm.Exponential("lambda", 1.0, shape=(data.shape[0], n_states))
            weight_stack = pm.math.sigmoid(np.random.randn(n_states, data.shape[1]))
            
            likelihood = PoissonLikelihood()
            obs = likelihood.generate_likelihood(data, lambda_vals, weight_stack)
    ...
    def test_gaussian_likelihood_2d(self):
        ...
        with pm.Model():
            ...
            obs = likelihood.generate_likelihood(data, (mu, sigma), weight_stack)
...
class TestComposableModel:
    ...
    def test_gaussian_fixed_sigmoid_model(self):
        ...
        changepoint_comp = FixedChangepoint(n_states, change_type='sigmoid')
        emission_comp = GaussianEmission(data.shape, n_states, include_variance=False)
...
class TestExampleFunctions:
    ...
    def test_create_model_function(self):
        ...
        model_instance = create_model('poisson_fixed_sigmoid', data, n_states=3)
        model = model_instance.generate_model()
